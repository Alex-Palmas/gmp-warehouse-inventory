...
export function RecordDetail({ session }: { session: Session }) {
  ...
  const rows: [string, string][] = [
    ...
    ['Receipt batch', rec.receiptBatchId || rec.serial],
    ['Record kind', rec.recordKind ?? 'container'],
    ['Parent serial', rec.parentSerial ?? ''],
    ['Qty received / current', `${rec.qtyReceived} / ${rec.currentQty} ${rec.uom}`],
    ['Qty per container', `${rec.qtyPerContainer ?? rec.qtyReceived} ${rec.uom}`],
    ['Containers', `${rec.containerIndex || 1} of ${rec.numberOfContainers} × ${rec.containerType}`],
    ...
    ['Reserved for request', rec.reservedForRequestId ? `${rec.reservedForRequestId} qty ${rec.reservedQty}` : ''],
